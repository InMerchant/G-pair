...
//웹툰 태그 불러오기
export async function updateWebtoonTag(webtoonID) {
  try {
    // 'webtoonDATA' 컬렉션에 대한 참조
    const webtoonCollectionRef = collection(db, "webtoonDATA");
    ...
    if (!querySnapshot.empty) {
      ...
      // HTML 요소에 웹툰 제목 업데이트
      document.getElementById('webtoon-tag').textContent = docData.genre;
      
    } else {
      console.log('No document matching the webtoonID.');
    }
  } catch (error) {
    console.error("Error fetching webtoon tag from Firestore: ", error);
  }
}