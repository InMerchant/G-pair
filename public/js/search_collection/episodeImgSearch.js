...
// 특정 웹툰 ID와 에피소드 ID에 해당하는 에피소드 이미지 데이터를 가져오는 함수(읽기용)
export async function getEpisodeImgData(webtoonID, episodeID, imgID) {
    try {
    ...
    if (!webtoonQuerySnapshot.empty) {
        ...
        if (!episodeQuerySnapshot.empty) {
            ...
            // 'imgID' 필드가 특정 imgID 값을 가진 문서를 찾는 쿼리
            const episodeImgQuery = query(epiImgCollectionRef, where("imgID", "==", imgID));
            
            // 쿼리 실행
            const epiImgQuerySnapshop = await getDocs(episodeImgQuery);

            // 에피소드 이미지 문서가 존재하는 경우
            if (!epiImgQuerySnapshop.empty) {
                // 에피소드 이미지 데이터 반환
                const episodeImgData = epiImgQuerySnapshop.docs[0].data();
                return episodeImgData;
            } else {
                console.log(`No episodeImage matching imgID ${imgID}`);
                return null;
            }
        } else {
...
// 특정 웹툰 ID와 에피소드 ID에 해당하는 에피소드 이미지 데이터를 가져오는 함수(쓰기용)
export async function getEpisodeImgDocData(webtoonID, episodeID, imgID) {
    try {
    // 'webtoonDATA' 컬렉션에 대한 참조
    const webtoonCollectionRef = collection(db, "webtoonDATA");
    ...
    if (!webtoonQuerySnapshot.empty) {
        ...
        if (!episodeQuerySnapshot.empty) {
            ...
            if (!epiImgQuerySnapshop.empty) {
                // 에피소드 이미지 데이터 반환
                const documentID = epiImgQuerySnapshop.docs[0].ref;
                return documentID;
            } else {
                console.log(`No episodeImage matching imgID ${imgID}`);
                return null;
